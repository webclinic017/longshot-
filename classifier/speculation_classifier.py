...
class SpeculationClassifier(AIClassifier):
    ...
    def training_set(self,ticker,prices,current):
        ...
        ticker_data["d1"] = ticker_data["adjopen"].pct_change(periods=1)
        ticker_data["d2"] = ticker_data["d1"].pct_change(periods=1)
        ticker_data["d3"] = ticker_data["d2"].pct_change(periods=1)
        ticker_data["rolling14"] = ticker_data["adjopen"].rolling(window=14).mean()
        ticker_data.dropna(inplace=True)
        ticker_data["ticker"] = ticker
        if not current:
            ticker_data["future"] = ticker_data["adjopen"].shift(-self.time_horizon_class.y_pivot_number)
